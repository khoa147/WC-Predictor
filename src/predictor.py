...
VN_TZ = timezone(timedelta(hours=7))
K_FACTOR = {"FIFA World Cup": 60, "UEFA Euro": 50, "Copa America": 50,
            "AFC Asian Cup": 45, "Africa Cup": 45, "friendly": 15, "default": 35}
DEFAULT_ELO = 1500
...
def get_weighted_form(rows: list, team: str, n: int = 8) -> dict:
    """Form có trọng số — trận gần đây tính nặng hơn"""
    matches = [r for r in rows
               if team in (r["home_team"], r["away_team"])
               and r["home_score"] not in ("", "NA")
               and r["away_score"] not in ("", "NA")][-n:]

    weights = [0.15, 0.15, 0.18, 0.18, 0.20, 0.20, 0.22, 0.25][-len(matches):]
    total_w = sum(weights)
    w_score = 0.0
    form_str = []
    ...
    for i, r in enumerate(matches):
        ...
        if is_home:
            ...
            form_str.append("W" if hs > as_ else ("D" if hs == as_ else "L"))
        else:
            result = 1.0 if as_ > hs else (0.5 if hs == as_ else 0.0)
            form_str.append("W" if as_ > hs else ("D" if hs == as_ else "L"))
        w_score += weights[i] * result

    form_score = w_score / total_w if total_w > 0 else 0.5
    return {"form_str": "".join(form_str) or "N/A", "form_score": form_score}


def get_h2h(rows: list, team1: str, team2: str, n: int = 10) -> dict:
    """Lịch sử đối đầu trực tiếp"""
    h2h = [r for r in rows
           if {r["home_team"], r["away_team"]} == {team1, team2}
           and r["home_score"] not in ("", "NA")][-n:]
    ...
    for r in h2h:
        ...
        if r["home_team"] == team1:
            if hs > as_: t1_wins += 1
            elif hs < as_: t2_wins += 1
            else: draws += 1
        else:
            if as_ > hs: t1_wins += 1
            elif as_ < hs: t2_wins += 1
            else: draws += 1

    total = t1_wins + t2_wins + draws
    if total == 0:
        return {"t1_win_pct": 0.33, "draw_pct": 0.33, "t2_win_pct": 0.33, "total": 0}
    return {
        "t1_win_pct": t1_wins / total,
        "draw_pct": draws / total,
        "t2_win_pct": t2_wins / total,
        "total": total,
    }


def predict_dixon_coles(elo_h: float, elo_a: float,
                        form_h: dict, form_a: dict,
                        h2h: dict, stage: str) -> dict:
    """
    Kết hợp ELO + Weighted Form + H2H + Tournament Context
    """
    # ── 1. Base probability từ ELO ──
    elo_win_prob = expected_score(elo_h, elo_a)
    diff = abs(elo_h - elo_a)

    # ── 2. Draw probability cơ bản ──
    # Trận cân bằng → hòa nhiều hơn
    draw_base = 0.26 - (diff / 5000)
    draw_prob = max(0.10, min(0.35, draw_base))

    # ── 3. Tournament context — World Cup vòng bảng ──
    # Đội yếu chơi phòng thủ hơn → tăng xác suất hòa
    is_group_stage = any(x in stage.lower() for x in ["group", "bảng", "matchday"])
    if is_group_stage and diff > 200:
        draw_prob = min(0.38, draw_prob + 0.08)  # tăng hòa khi chênh lệch lớn ở vòng bảng

    # ── 4. Form adjustment ──
    form_diff = form_h["form_score"] - form_a["form_score"]
    form_adjustment = form_diff * 0.12  # form ảnh hưởng 12%

    # ── 5. H2H adjustment ──
    h2h_adjustment = 0.0
    if h2h["total"] >= 3:
        h2h_base = h2h["t1_win_pct"] - h2h["t2_win_pct"]
        h2h_adjustment = h2h_base * 0.10  # H2H ảnh hưởng 10%

    # ── 6. Tổng hợp ──
    # Trọng số: ELO 65% + Form 12% + H2H 10% + base 13%
    home_raw = elo_win_prob + form_adjustment + h2h_adjustment
    home_raw = max(0.05, min(0.90, home_raw))

    # Phân phối W/D/L
    if home_raw > 0.5:
        home_win = home_raw - draw_prob / 2
        away_win = 1 - home_win - draw_prob
    else:
        away_win = (1 - home_raw) - draw_prob / 2
        home_win = 1 - away_win - draw_prob

    home_win = max(0.05, home_win)
    away_win = max(0.05, away_win)
    draw_prob = max(0.08, draw_prob)
...
def parse_match_to_vn(m: dict) -> dict:
    ...
    return {
        "home": m["team1"], "away": m["team2"],
        "time": local_time, "date_vn": vn_date,
        "stage": m.get("group", m.get("round", "World Cup 2026")),
    }
...
def analyze_with_gemini(home, away, elo_h, elo_a, form_h, form_a, h2h, probs, stage):
    import time
    h2h_str = f"{h2h['total']} trận: {home} thắng {round(h2h['t1_win_pct']*100)}% | Hòa {round(h2h['draw_pct']*100)}% | {away} thắng {round(h2h['t2_win_pct']*100)}%" if h2h["total"] > 0 else "Chưa có lịch sử"

    prompt = f"""World Cup 2026: {home} vs {away} ({stage})
ELO: {home}={round(elo_h)} | {away}={round(elo_a)}
Form gần đây (có trọng số): {home}={form_h['form_str']} | {away}={form_a['form_str']}
Đối đầu trực tiếp: {h2h_str}
Xác suất tổng hợp: {home} thắng {probs['home_win_pct']}% | Hòa {probs['draw_pct']}% | {away} thắng {probs['away_win_pct']}%

Viết 2 câu nhận xét ngắn tiếng Việt dựa trên dữ liệu trên. Chỉ trả lời JSON:
{{"comment":"2 câu nhận xét","scoreline":"X-Y","confidence":{round(max(probs['home_win_pct'],probs['away_win_pct']))}}}"""
...
def format_message(results: list, dates: list) -> str:
    ...
    for i, item in enumerate(results, 1):
        ...
        if r.get("h2h_total", 0) > 0:
            lines.append(f"🤝 H2H: {home} {round(r['h2h']['t1_win_pct']*100)}% | Hòa {round(r['h2h']['draw_pct']*100)}% | {away} {round(r['h2h']['t2_win_pct']*100)}%")
        if ai.get("comment"):
            lines += ["", f"💬 _{ai['comment']}_"]
        lines.append("━━━━━━━━━━━━━━━━━━")

    lines += ["", "🤖 _ELO + Form + H2H + Dixon-Coles_", "_⚠️ Dự đoán tham khảo!_"]
    return "\n".join(lines)
...
def main():
    ...
    for idx, match in enumerate(matches):
        ...
        elo_h = elo_ratings.get(home, DEFAULT_ELO)
        elo_a = elo_ratings.get(away, DEFAULT_ELO)
        form_h = get_weighted_form(history, home)
        form_a = get_weighted_form(history, away)
        h2h = get_h2h(history, home, away)
        probs = predict_dixon_coles(elo_h, elo_a, form_h, form_a, h2h, match["stage"])

        print(f"  ELO {home}={round(elo_h)} | {away}={round(elo_a)}")
        print(f"  Form: {home}={form_h['form_str']} | {away}={form_a['form_str']}")
        print(f"  H2H: {h2h['total']} trận | Xác suất: {probs}")

        try:
            ai = analyze_with_gemini(home, away, elo_h, elo_a, form_h, form_a, h2h, probs, match["stage"])
            print(f"  ✅ {ai.get('scoreline')} ({ai.get('confidence')}%)")
        except Exception as e:
            print(f"  ❌ Gemini lỗi: {e}")
            ai = {"comment": "", "scoreline": "?-?",
        ...
        results.append({"match": match, "result": {
            "probs": probs, "ai": ai,
            "elo_home": elo_h, "elo_away": elo_a,
            "form_home": form_h["form_str"],
            "form_away": form_a["form_str"],
            "h2h": h2h,
            "h2h_total": h2h["total"],
        }})
